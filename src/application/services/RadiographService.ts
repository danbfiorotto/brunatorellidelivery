...
export class RadiographService {
    ...
    async getRadiographs(patientId: string): Promise<Radiograph[]> {
        try {
            ...
            throw this.errorHandler.handle(error, { context: 'RadiographService.getRadiographs', patientId });
        }
    }
    ...
    async uploadRadiograph(
        ...
        try {
            ...
            await this.auditService.log('create', 'radiograph', data.id, null, { patientId, appointmentId });
            logger.debug('Radiograph uploaded successfully', { radiographId: data.id });

            return data;
        } catch (error) {
            throw this.errorHandler.handle(error, { context: 'RadiographService.uploadRadiograph', patientId, appointmentId });
        }
    }
    ...
    async deleteRadiograph(id: string): Promise<void> {
        try {
            ...
            await this.auditService.log('delete', 'radiograph', id, oldData, null);
            logger.debug('Radiograph deleted successfully', { radiographId: id });
        } catch (error) {
            throw this.errorHandler.handle(error, { context: 'RadiographService.deleteRadiograph', id });
        }
    }
    ...
    async uploadFileToStorage(file: File, folder: string = 'radiographs'): Promise<UploadResult> {
        try {
            ...
            throw this.errorHandler.handle(error, { context: 'RadiographService.uploadFileToStorage', folder });
        }
    }
}
